<?php
  ...
  if(isset($_POST["submit"])){
    ...
    if(!$error){
      ...
      require_once("db.php");
      $sql = "SELECT employeeEmail FROM employee WHERE employeePassword='$employeePassword' AND employeeType = 'employee'";
      ...
      if($loginOK) {
        ...
        Header("Location:LandingPageEmp_Keith.php"); //access php page session start.
      }
    }
...
<body>
  ...
  <p>To create an account, contact a manager. </p>
    ...
    <th width="40" colspan="2">
      ...
      <tr>
        <td>Manager Email</td>
        <td><input type="email" placeholder="example@email.com" name="employeeEmail" min="1" max="29" value="<?php //USE THIS TO SAVE NAMES FOR REVIEW PAGE
      ...
      <tr>
        <td>Password</td>
        <td><input type="password" placeholder="*********" name="employeePassword" value="<?php if(!empty($employeePassword)) echo $employeePassword; ?>" /><?php if($error && empty($employeePassword)) echo "<span class='errlabel'> please enter a password</span>"; ?></td>
      </tr>
    </table>
<br>
